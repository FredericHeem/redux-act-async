...
describe('createActionAsync', function () {
  ...
  it('options',  () => {
    ...
    const login = createActionAsync(actionName, apiOk, option);
    assert(login.options);
    assert(login.options.request.payloadReducer);
    assert(login.options.request.metaReducer);

  });
  it('callback in options', function () {
    let actionName = 'LOGIN_11';
    let loginUser = {
      username: 'ciccio',
      password: 'password'
    };
    ...
    function apiOk(username, password){
      if(password === 'password'){
        ...
        return Promise.reject({name:"InvalidPassword"});
      }
    }
    ...
    store.dispatch(login(loginUser.username, loginUser.password));
    store.dispatch(login(loginUser.username, "babpassword"));
  });
});
